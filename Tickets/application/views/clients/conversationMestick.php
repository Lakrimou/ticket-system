<div class="content" id="ticket" data-id="343" data-access="6xCMzlBEG3">
		<div class="page-title-cont clearfix">
			...
			?>
			<div class="badge red">OUVERT</div>	
			<?php }
			else{ ?>
				<div class="badge green">FERME</div>
			<?php } ?>		
					</div>
		
		<div class="row">
			
			<div class="col margin-top no-bottom-padding col-md-12 ticket">
				 <?php
								$titre;
								$body;
								if($openTicketsParent){
									foreach ((array)$openTicketsParent as $key) {
										$titre = $key['title'];
										$body = $key['body'];
									}
								}if($ClosedTicketsParent){
									foreach ((array)$ClosedTicketsParent as $key) {
											$titre = $key['title'];
											$body = $key['body'];
									}
								}
					...
					<div class="col col-xs-12">
						<div class="cont">
							<div class="top">
								...
								<p>
									<?php echo '<span class="glyphicon glyphicon-alert" aria-hidden="true"></span> '.$body;?>
								</p>
							</div>
							<!-- <div>
								<img src="<?php echo base_url('assets/images/avatar.PNG')?>" style="display: inline-block;">
							</div> -->
							...
							<div style="display: inline-block;">
								...
								if($conversation) {								
									foreach ($conversation as $key) {
										...
										?>
										<img src="<?php echo base_url('assets/images/avatar.PNG');?>" style="display: inline-block;">
										<?php echo '<div class="tick-response" style="display: inline-block;" > <strong>'.$key['username'].'</strong> '.$key['body'].'</div><br/>'; 
									}}
								}else {
									echo ' ';
								}
							...
							<?php echo form_open("userAdmin/repondreTicket", array('class' => 'form-horizontal'));?>
      ...
      <div class="form-group text-right">
          <div class="col-sm-12"><?php echo form_submit('repondre', "Répondre", 'class="btn btn-primary"');?></div>
      </div>
      <?php echo form_close();?>
						</div>
					</div></div>
